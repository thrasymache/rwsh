...
int cd_bi(const Argv_t& argv) {
  if (argv.size() != 2) {vars->set("ERRNO", "ARGS"); return -1;}
  errno = 0;
  int ret = chdir(argv[1].c_str());
  if (!ret) vars->set("CWD", argv[1]);
  else if (errno == ENOENT) vars->set("ERRNO", "NOENT");
  else if (errno == ENOTDIR) vars->set("ERRNO", "NOTDIR"); 
  else vars->set("ERRNO", "CD_ERROR"); 
  errno = 0;
  return ret;}
...
static int if_core(const Argv_t& argv, bool logic) {
  ...
  if (logic == !executable_map[lookup](lookup)) {
    ...
    if (vars->get("IF_TEST") != "") {
      vars->set("ERRNO", "BAD_IF_NEST"); ret = -1;}
    vars->set("IF_TEST", "true");
    return ret;}
  else return 0;}

// run argfunction if $* returns true
int if_bi(const Argv_t& argv) {
  if (vars->get("ERRNO") != "") return dollar_question;
  else if (argv.size() < 2) {vars->set("ERRNO", "ARGS"); return -1;}
  else if (vars->get("IF_TEST") == "") {
    vars->set("IF_TEST", "false"); 
    return if_core(argv, true);}
  else {vars->set("ERRNO", "IF_BEFORE_ELSE"); return -1;}}

// run argfunction if ERRNO is set
int if_errno_bi(const Argv_t& argv) {
  int ret = 0;
  if (vars->get("ERRNO") != "" && argv.argfunction()) 
    ret = (*argv.argfunction())(Argv_t("rwsh.mapped_argfunction"));
  if (argv.size() != 1) {
    vars->set("ERRNO", "ARGS");
    if (argv.argfunction()) 
...
int else_if_bi(const Argv_t& argv) {
  ...
  else if (vars->get("IF_TEST") == "true") return dollar_question;
  else if (vars->get("IF_TEST") == "false") return if_core(argv, true);
  else {vars->set("ERRNO", "ELSE_WITHOUT_IF"); return -1;}}
...
int else_if_not_bi(const Argv_t& argv) {
  ...
  else if (vars->get("IF_TEST") == "false") return if_core(argv, false);
  else {vars->set("ERRNO", "ELSE_WITHOUT_IF"); return -1;}}

// run argfunction if IF_TEST is false 
int else_bi(const Argv_t& argv) {
  int ret;
  if (vars->get("ERRNO") != "") ret = dollar_question;
  else if (argv.size() != 1) {vars->set("ERRNO", "ARGS"); ret = -1;}
  else if (vars->get("IF_TEST") == "true") ret = dollar_question;
  else if (vars->get("IF_TEST") == "false") 
    if (argv.argfunction()) {
      vars->set("IF_TEST", "");
      (*argv.argfunction())(Argv_t("rwsh.mapped_argfunction"));
      if (vars->get("IF_TEST") != "") {vars->set("ERRNO", "BAD_IF_NEST"); ret = -1;}
      else ret = dollar_question;}
    else ret = 0;
  else {vars->set("ERRNO", "ELSE_WITHOUT_IF"); ret = -1;}
  vars->set("IF_TEST", "");
  return ret;}

// exit the shell
int exit_bi(const Argv_t& argv) {
  if (argv.size() != 1) {vars->set("ERRNO", "ARGS"); return -1;}
...
int for_bi(const Argv_t& argv) {
  ...
  for (Argv_t::const_iterator i = ++argv.begin(); i != argv.end(); ++i) {
    if (argv.argfunction()) {
      ...
      if (Executable_t::unwind_stack() || vars->get("ERRNO") != "") return -1;}
    else ret = 0;}
  return ret;}
...
int importenv_overwrite_bi(const Argv_t& argv) {
  ...
  for (char** i=environ; *i; ++i) {
    ...
    if (div != std::string::npos) 
      vars->set(src.substr(0, div), src.substr(div+1));}
  return 0;}
...
int importenv_preserve_bi(const Argv_t& argv) {
  ...
  for (char** i=environ; *i; ++i) {
    std::string src(*i);
    std::string::size_type div = src.find("=");
    if (div != std::string::npos && vars->get(src.substr(0, div)) == "") 
      vars->set(src.substr(0, div), src.substr(div+1));}
  return 0;}
...
int return_bi(const Argv_t& argv) {
  ...
  if (!*focus || *endptr) {vars->set("ERRNO", "NAN"); return -1;}
  if (errno == ERANGE) vars->set("ERRNO", "RANGE");
  if (errno == EINVAL) {vars->set("ERRNO", "INVAL"); return -1;}
  else if (ret < INT_MIN) {vars->set("ERRNO", "RANGE"); return INT_MIN;}
  else if (ret > INT_MAX) {vars->set("ERRNO", "RANGE"); return INT_MAX;}
  else if (errno) {vars->set("ERRNO", "RETURN_ERROR"); return -1;}
  else return ret;}

// modify variable $1 as a selection according to $2
int selection_set_bi(const Argv_t& argv) {
  if (argv.size() < 3) {vars->set("ERRNO", "ARGS"); return -1;}
  std::vector<Entry_pattern_t> focus;
  str_to_entry_pattern_vector(vars->get(argv[1]), focus);
  for (Argv_t::const_iterator i = argv.begin()+2; i != argv.end(); ++i) 
    str_to_entry_pattern_vector(*i, focus);
  vars->set(argv[1], entry_pattern_vector_to_str(focus));
  return 0;}
...
int source_bi(const Argv_t& argv) {
  if (argv.size() < 2) {vars->set("ERRNO", "ARGS"); return -1;}
  struct stat sb;
  if (stat(argv[1].c_str(), &sb)) {vars->set("ERRNO", "NOENT"); return -1;}
  if (!(sb.st_mode & S_IXUSR)) {vars->set("ERRNO", "NOEXEC"); return -1;}
...
int stepwise_bi(const Argv_t& argv) {
  if (argv.size() < 2) {vars->set("ERRNO", "ARGS"); return -1;}
  if (!argv.argfunction()) {vars->set("ERRNO", "ARGFUNCTION"); return -1;}
  Argv_t lookup(argv.begin()+1, argv.end(), 0);
  ...
  for (Function_t::const_iterator i = f->script.begin(); 
    ...
    ret  = (*argv.argfunction())(body);
    if (Executable_t::unwind_stack() || vars->get("ERRNO") != "") {
      ret = -1;
      break;}}
...
int which_path_bi(const Argv_t& argv) {
  ...
  tokenize_strict(vars->get("PATH"), std::back_inserter(path), 
